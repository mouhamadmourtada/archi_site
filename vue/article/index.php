...
<div class="my-5 p-10">
    ...
    <div class="w-full scroll mx-auto ">
                ...
                <td class="py-2 px-2 border border-gray-200">
                    <div class="flex justify-around text-sm">
                        <div class="btn btn-primary mx-1 rounded my-3 hover:shadow">
                            <a href="/archi/article/<?= $article->getId() ?>/show" >voir</a>
                        </div>
                        <div class="btn btn-primary mx-1 rounded my-3 hover:shadow">
                            <a href="/archi/article/<?= $article->getId() ?>/edit" >Modifier</a>
                        </div>
                        <div class="btn btn-primary mx-1 rounded my-3 hover:shadow">
                            <a href="/archi/article/<?= $article->getId() ?>/delete" >Supprimer</a>
                        </div>
                    </div>
                </td>
            </tr>
       ...
       <div class="pagination mt-5 flex justify-end">
            <?php if ($page > 1): ?>
                <a  href="/archi/article?page=<?= ($page - 1) ?>&size=<?= $size ?>" class="btn btn-primary bg-slate-200 hover:bg-slate-300 py-1 px-3 rounded mx-2 ">Précédent</a>
            <?php endif; ?>

            <?php if (($page - 1) * $size + count($articles) < $count): ?>
                <a href="/archi/article?page=<?= ($page + 1) ?>&size=<?= $size ?>" class="btn btn-primary bg-slate-200 hover:bg-slate-300 py-1 px-3 rounded mx-2">Suivant</a>
            <?php endif; ?>
        </div>
